...
class Distribution(object):
    ...
    def from_file(settings, filename):
        if any(map(filename.endswith, ['.z64', '.n64', '.v64'])):
            raise RuntimeError("Your Ocarina of Time ROM doesn't belong in the plandomizer setting. If you don't know what plandomizer is, or don't plan to use it, leave that setting blank and try again.")

        try:
            with open(filename) as infile:
                src_dict = json.load(infile)
        except json.decoder.JSONDecodeError as e:
            raise RuntimeError("Invalid Plandomizer File. Make sure the file is a valid JSON file.") from None
